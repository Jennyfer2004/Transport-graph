...
def get_random_closest_edge(pos, current_node, previous_node):
    ...
    for edge in adjacent_edges:
        ...
        if next_node == previous_node:
            continue  # Evitar moverse hacia atrás
...
def main():
    ...
    if agree:
        total_weights_by_col = {}
        total_weights_by_fil = {}
        ...
        for edge, weight in edge_weights.items():
            ...
            if x0 == x1:
                fil = x1  
                if fil not in total_weights_by_fil:
                    total_weights_by_fil[fil] = weight
            elif y0 == y1:
                col = y0  # Determinar la columna basada en la posición de las aristas
                if col not in total_weights_by_col:
                    total_weights_by_col[col] = weight
        
        fil = st.sidebar.checkbox("Dimensiones de las avenidas")
        if fil:
            st.sidebar.subheader("Pesos Totales por Columna")
            for col, total_weight in total_weights_by_col.items():
        ...
        if col:
            ...
            for fil, total_weight in total_weights_by_fil.items():
